...
function EmployeeOptionsPanel({ open, query, setQuery, filtered, notLinkedLabel }) {
...
/**
 * Searchable linked-employee picker (same panel pattern as {@link CountryCombobox}).
 *
 * @param {object} props
 * @param {string} props.id
 * @param {number|string|''|null|undefined} props.value — employee id or empty
 * @param {Array<{ id: number, employee_code?: string, display_name?: string, branch_id?: number }>} props.employees
 * @param {(employee: object | null) => void} props.onChange — `null` when cleared
 * @param {string} [props.notLinkedLabel]
 * @param {string} [props.placeholder]
 * @param {string} [props.className]
 */
export default function LinkedEmployeeCombobox({
    id,
    value,
    employees,
    onChange,
    ...
    const optionItems = useMemo(
        () => [NOT_LINKED, ...employees.map((e) => ({ _kind: 'emp', ...e }))],
        [employees],
    );
    ...
    const filtered = useMemo(() => {
        ...
        return optionItems.filter((o) => {
            ...
            const code = (o.employee_code || '').toLowerCase();
            const name = (o.display_name || '').toLowerCase();
    ...
    const selected = useMemo(() => {
        ...
        const found = employees.find((e) => Number(e.id) === idNum);
        if (!found) {
            return NOT_LINKED;
        }
        return { _kind: 'emp', ...found };
    }, [value, employees]);

    const displayLabel =
        selected._kind === 'emp' ? formatRow(selected) : null;
