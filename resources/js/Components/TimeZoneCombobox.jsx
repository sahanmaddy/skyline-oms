...
function offsetSortKey(offset) {
    const m = offset.match(/^(?:GMT\s*)?([+-])(\d{2})\.(\d{2})$/);
    if (!m) return 0;
    const sign = m[1] === '-' ? -1 : 1;
...
function TimeZoneOptionsPanel({ open, query, setQuery, filtered }) {
    ...
    return (
        ...
        >
            ...
            <div className="max-h-52 overflow-y-auto py-1">
                {filtered.map((opt) => (
                    <ListboxOption key={opt.value} value={opt} className={dropdownMenuItemClass}>
                ...
                {filtered.length === 0 && (
                    <div className="px-4 py-2 text-sm text-gray-500 dark:text-cursor-muted">No matches.</div>
                )}
            </div>
        </ListboxOptions>
    );
...
export default function TimeZoneCombobox({
    ...
    const normalized = useMemo(() => {
        return (options || [])
            .map((tz) => ({
                ...
                offset: formatOffsetLabel(tz),
            }))
            .sort((a, b) => {
                const byOffset = offsetSortKey(a.offset) - offsetSortKey(b.offset);
    ...
    const filtered = useMemo(() => {
        ...
        return normalized.filter((o) => {
            return (
                o.value.toLowerCase().includes(raw) ||
                o.label.toLowerCase().includes(raw) ||
                o.offset.toLowerCase().includes(raw)
            );
        });
    }, [normalized, query]);
    ...
    return (
        <Listbox value={selected} onChange={(opt) => onChange(opt?.value || '')} by={optionsByComparator} disabled={disabled}>
            {({ open }) => (
