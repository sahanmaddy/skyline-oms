            ...
            <ProcurementModuleLayout breadcrumbs={[{ label: 'Suppliers', href: route('procurement.suppliers.index') }, { label: supplier.display_name || 'Supplier' }]}>
                <div className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                        <section className="rounded-lg border border-gray-200 bg-white p-5"><h3 className="text-sm font-semibold text-gray-900">Address</h3><div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2"><Info label="Address" value={[supplier.address_line_1, supplier.address_line_2].filter(Boolean).join(', ') || '—'} className="sm:col-span-2" /><Info label="City" value={supplier.city || '—'} /><Info label="State / Province" value={supplier.state_province || '—'} /><Info label="Postal Code" value={supplier.postal_code || '—'} /><Info label="Country" value={supplier.country || '—'} /></div></section>
                        <section className="rounded-lg border border-gray-200 bg-white p-5"><h3 className="text-sm font-semibold text-gray-900">Business Details</h3><div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2"><Info label="Registration Number" value={supplier.registration_number || '—'} /><Info label="Tax Number" value={supplier.tax_number || '—'} /><Info label="Payment Terms (days)" value={supplier.payment_terms_days ?? '—'} /><Info label="Credit Limit" value={supplier.credit_limit ?? '—'} /></div></section>
                    </div>
