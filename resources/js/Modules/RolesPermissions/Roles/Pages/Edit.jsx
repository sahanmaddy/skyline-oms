            ...
            <SettingsModuleLayout breadcrumbs={[{ label: 'Roles', href: route('settings.roles.index') }, { label: role.name, href: route('settings.roles.show', role.id) }, { label: 'Edit' }]}>
                <div className="space-y-4">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                        <RoleForm data={data} setData={setData} errors={errors} processing={processing} permissionGroups={permissionGroups} submitLabel="Save changes" onSubmit={() => put(route('settings.roles.update', role.id))} />
                    </div>
                </div>
