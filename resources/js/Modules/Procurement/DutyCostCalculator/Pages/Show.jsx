            ...
            >
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <div className="mt-1 text-sm text-gray-600">
                                Ref: {calculation.reference_no || '—'} · Supplier: {calculation.supplier_name || '—'}
                            </div>
                ...
                <section className="rounded-lg border border-gray-200 bg-white p-5">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                        {[
                            ['Exchange Rate', calculation.exchange_rate],
                            ['Rate Label', calculation.exchange_rate_currency_label || '—'],
                            ['Container CBM Capacity', calculation.container_cbm_capacity || '—'],
                            ['Shipping Cost', formatCompanyCurrency(calculation.shipping_cost_total_lkr, company)],
                            ['Other Costs Total', formatCompanyCurrency(calculation.other_costs_lkr_total, company)],
                            ['Status', calculation.calculation_status],
                            ['Total Weight', totals.total_weight_kg ?? calculation.total_weight_kg],
                            ['Total CBM', totals.total_cbm ?? calculation.total_cbm],
                            ['Shipping per CBM', formatCompanyCurrency(totals.shipping_cost_per_cbm_lkr ?? calculation.shipping_cost_per_cbm_lkr, company)],
                        ].map(([k, v]) => (
                ...
                <section className="overflow-hidden rounded-lg border border-gray-200 bg-white">
                    <div className="overflow-x-auto">
                        <table className="min-w-[1800px] divide-y divide-gray-200 text-sm">
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {(calculation.items || []).map((row) => (
                                    <tr key={row.id}>
                                        ...
                                        <td className="px-3 py-2">{row.product_currency}</td>
                                        ...
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.product_value_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{row.weight_kg}</td>
                                        <td className="px-3 py-2 text-right">{row.cbm}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.statistical_value_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.cid_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.vat_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.sscl_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.duty_total_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.allocated_shipping_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right">{formatCompanyCurrency(row.allocated_other_costs_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right font-semibold">{formatCompanyCurrency(row.total_landed_cost_lkr, company)}</td>
                                        <td className="px-3 py-2 text-right font-semibold">{formatCompanyCurrency(row.landed_cost_per_unit_lkr, company)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <section className="rounded-lg border border-gray-200 bg-white p-5">
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                        {[
                            ['Total Product Value (LKR)', totals.total_product_value_lkr],
                            ...
                            ['Total Allocated Shipping', totals.total_allocated_shipping_lkr],
                            ...
                            <div key={label} className="rounded-md border border-gray-100 bg-gray-50 p-3">
                                ...
                                <div className="mt-1 text-sm font-semibold text-gray-900">
                                    {formatCompanyCurrency(value || 0, company)}
                                </div>
                            </div>
                        ))}
                    </div>
