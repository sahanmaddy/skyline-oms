...
export default function ValueCreate({ typeOptions, prefillTypeId }) {
    const toast = useToast();
    const initialType =
        prefillTypeId && typeOptions?.some((t) => Number(t.id) === Number(prefillTypeId))
            ? String(prefillTypeId)
            : typeOptions?.[0]?.id
              ? String(typeOptions[0].id)
              : '';

    const form = useForm({
        product_attribute_type_id: initialType,
        value: '',
        display_value: '',
        description: '',
        status: 'active',
    ...
    return (
        <AuthenticatedLayout header={<ModuleStickyTitle module="Inventory" section="New attribute value" />}>
            <Head title="New attribute value · Inventory" />
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-cursor-border dark:bg-cursor-surface">
                        <form
                            onSubmit={(e) => {
                                e.preventDefault();
                                form.post(route('inventory.attribute-values.store'), {
                                    preserveScroll: true,
                                    onError: () => {
                                        scrollToFirstError();
